...
```
*Observation*: the highest value for rating decreased.
...
In order to exclude the movies which have the least number of votes, we could filter them by taking only those which do not exceed a certain big rating. Those with the biggest rating usually have the least votes so they are irrelevant.
...
ggplot(father.son, aes(fheight, sheight)) + geom_point() + geom_smooth(method =
                                                                         "lm", colour = "red") + geom_abline(slope = 1, intercept = 0)
...
- Seniority, meaning months since first hired, is not necessary connected to the Age of the employee or his/her Experience. It only displays the amount of time spent working after employment with the bank, not prior (the Experience variable). That's why the correlation coefficient is so low (there is no linear correlation between them).
...
### Exercise 10. Hertzsprung-Russell

![](./exercises/exercise_2_10.PNG)
...
ggplot(HRstars, aes(BV, V)) + geom_point()
